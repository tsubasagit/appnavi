/**
 * ブラウザコンソールから実行する強化版CRMテンプレート更新スクリプト
 * 
 * 使用方法:
 * 1. AppNaviアプリケーションに管理者（admin）でログイン
 * 2. ブラウザの開発者ツール（F12）を開く
 * 3. コンソールタブでこのスクリプトをコピー＆ペーストして実行
 * 4. updateEnhancedCRMTemplate() を実行
 */

async function updateEnhancedCRMTemplate() {
  try {
    console.log('=== 強化版CRMテンプレート更新スクリプト開始 ===')
    ...
    console.log('✓ 強化版CRMテンプレートを更新しました！')
    ...
    console.log('=== 強化版CRMテンプレート更新完了 ===')
    
    return {
      success: true,
      message: '強化版CRMテンプレートを更新しました',
      pages: cleanedTemplateData.uiStructure.pages.map(p => p.name)
    }
  } catch (error) {
    console.error('❌ 強化版CRMテンプレート更新エラー:', error)
    console.error('エラー詳細:', {
      name: error?.name,
...
// スクリプトを実行
console.log('強化版CRMテンプレート更新スクリプトを読み込みました。')
console.log('実行するには: updateEnhancedCRMTemplate()')
console.log('')
console.log('このスクリプトは、CRMテンプレートを6つのページ（ダッシュボード、顧客管理、商談管理、売上分析、タスク・フォローアップ、活動履歴）で更新します。')
